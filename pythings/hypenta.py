...
def getsubject():
    ...
    for i in range(0, allpages):
        ...
        get = requests.get(
            url='https://v.huya.com/g/vhuyawzry?set_id=3&order=new&page=' + str(allpages - page))
        get.encoding = 'utf-8'
        print('get', get.text)
        selector = etree.HTML(get.text)
        # print('selector',selector)
        # 当前所访问页面的长度
        pagelength = selector.xpath(
            "/html/body/div[1]/div/article/section/ul[2]/li")
...
def findtext(selector, index, lenpage):
    ...
    for i in range(0, lenpage):
        print("当前整体selector", selector)
        title = selector.xpath(
            '/html/body/div[1]/div/article/section/ul[2]/li[' + str(lenpage - item) + ']/a/p/text()')
        if title == []:
            print("这个b标题为空")
            title.append("标题为空")
        ...
        if(result):
            print("执行提取进一步五杀信息")
            new_titles = title[0]
            new_href = str("https:" + href[0])

            getitem = requests.get(url="https:" + href[0])
            getitem.encoding = 'utf-8'

            selectoritem = etree.HTML(getitem.text)

            itemtime = selectoritem.xpath(
                '//*[@id="play2"]/div[2]/div[1]/div[2]/div[1]/div[1]/p[1]/span[2]/text()')
            author = selectoritem.xpath(
                '//*[@id="play2"]/div[2]/div[1]/div[2]/div[1]/div[2]/a[1]/h3/text()')
            ...
            if(comparetime(itemtime[0])):
                ...
                print("最新数据更新")
                dbinsert.insertpenta(new_titles, new_href,
                                     author[0], itemtime[0], new_imgurl, table)
